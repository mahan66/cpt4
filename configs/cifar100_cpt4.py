...
def get_args_parser(subparsers):
    ...
    subparsers.add_argument('--pretrained', action='store_true', help='Load pretrained model or not')
    ...
    subparsers.add_argument('--nb_classes', type=int, default=100, help='Number of distinct classes')


    subparsers.add_argument('--use_batch_norm', action='store_true', help='Use BatchNorm instead of LayerNorm or not')
    ...
    subparsers.add_argument('--lr', type=float, default=1e-3, metavar='LR', help='learning rate (default: 0.03)')
    ...
    subparsers.add_argument('--lr_step_size', type=int, default=10, help='lr step size in scheduler')
    subparsers.add_argument('--lr_gamma', type=float, default=0.5, help='rate of change in lr in each step')
    ...
    subparsers.add_argument('--prompt_pool', action='store_true')
    ...
    subparsers.add_argument('--pretrain_ratio', type=float, default=0.001, help='The ratio of pre-training examples')
    subparsers.add_argument('--model_file_name', default=None, type=str, help='pretrained model file name')
    subparsers.add_argument('--model_output_name', default=None, type=str, help='pretrained model output file name')
    subparsers.add_argument('--model_dir_name', default='pretrained-models', type=str, help='pretrained model dir name')

    # Other Parameters
    subparsers.add_argument('--method_name', default='cpt4', type=str, help='the method name')

    subparsers.add_argument('--batched_layers', default=['patch_embed', 'mlp'], nargs='+', type=str, help='add batch_norm in the batched layers')
    subparsers.add_argument('--train_trainable_layers', default=('head', 'prompt', 'patch_embed.batch_norm'), nargs='+', type=str, help='consider the parameters of these layers trainable in training')

    subparsers.add_argument('--test_data_ratio', default='1', type=float, help='The ratio of Test Time examples')
    subparsers.add_argument('--test_trainable_layers', default=['head', 'prompt', 'patch_embed.batch_norm'], nargs='+', type=str, help='consider the parameters of these layers trainable in test-time')
    subparsers.add_argument('--adaptation_strategy', default='none', type=str, help='the adaptation strategy name')
    subparsers.add_argument('--gradual', action='store_true', help='if set, testing the gradual test time corruptions')
    subparsers.add_argument('--shuffle_corruption_types', action='store_true', help='shuffle the corruption types list')
    subparsers.add_argument('--corruption_types',
                            default=['gaussian_noise', 'shot_noise', 'impulse_noise', 'defocus_blur',
                                     'glass_blur', 'motion_blur', 'zoom_blur', 'snow', 'frost', 'fog',
                                     'brightness', 'contrast', 'elastic_transform', 'pixelate', 'jpeg_compression'],
                            nargs='+', type=str, help='all the corruption types for CIFAR datasets')
