...
Revised By Mahmood Karimian November 2023
# Add different Batch Norm layers strategies
"""
import math
import logging
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.head_type = head_type if prompt_pool else 'token'
    ...
    def replace_LayerNorm_with_BatchNorm(self):
        # replace all the LayerNorms with BatchNorm
        self.norm = MyBatchNorm(self.norm.normalized_shape)

        for name, module in self.named_modules():
            if isinstance(module, Block):
                module.norm1 = MyBatchNorm(module.norm1.normalized_shape)
                module.norm2 = MyBatchNorm(module.norm2.normalized_shape)
                module.mlp.norm = MyBatchNorm(module.mlp.fc1.out_features)
                setattr(module.mlp, '__class__', MyMlp)

    def add_batch_norm(self, layer_names='patch_embed'):
        for name, module in self.named_modules():
            if isinstance(module, PatchEmbed) and 'patch_embed' in layer_names:
                module.batch_norm = nn.BatchNorm2d(module.proj.out_channels)
                setattr(module, '__class__', MyPatchEmbed)
            elif isinstance(module, Block):
                if 'mlp' in layer_names:
                    module.mlp.batch_norm = MyBatchNorm(module.mlp.fc1.out_features)
                    setattr(module.mlp, '__class__', MyMlp)
                # if 'norm1' in layer_names:
                #     module.norm1 = MyBatchNorm(module.norm1.normalized_shape)
                # if 'norm2' in layer_names:
                #     module.norm2 =
...
class MyPatchEmbed(PatchEmbed):
    # def __init__(self):
    #     super().__init__()
    #     self.bacth_norm = nn.BatchNorm2d(self.proj.out_channels)
    ...
    def forward(self, x):
        ...
        x = self.batch_norm(x)
        if self.flatten:
            x = x.flatten(2).transpose(1, 2)  # BDN_pN_p -> B * (N_p * N_p) * D
        x = self.norm(x)
        return x

class MyBatchNorm(nn.BatchNorm1d):
    """
    Apply BatchNorm1d on input size (B, N, C)
    B: number of Batchs
    N: feature size
    ...
    def forward(self, x):
        ...
        x = super().forward(x)  # Apply BatchNorm1d to the input
        x = x.reshape(B, N, C)
        return x

class MyMlp(Mlp):
    """
    Add one BatchNorm between two linear layers
    """

    def forward(self, x):
        x = self.fc1(x)
        x = self.batch_norm(x)  # a MyBatchNorm will be defined later for this layer
        x = self.act(x)
        x = self.drop1(x)
        x = self.fc2(x)
        x = self.drop2(x)
